...
<template>
  ...
  <div v-if="isRegistered">
    ...
    <ul
      class="hidden text-sm my-4 font-medium text-center text-gray-500 rounded-lg shadow sm:flex dark:divide-gray-700 dark:text-gray-400"
      id="tabList">
  ...
  <div class="space-y-1">
    <div class="rounded-md border bg-white">
      <div>
        <div class="m-16 text-center" v-if="searchActivities.length == 0">
          <div class="text-2xl">No activity</div>
          <div>
            <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 px-6 py-2 pt-8 pb-8">
              <div v-for="(activity, index) in searchActivities" :key="index">
                <div class="max-w-sm h-full rounded overflow-hidden shadow-lg ">
                  <div @click="navigateTo(`/Activities/${activity.id}/`)" class="h-full">
                    <div @click="trackHistory(activity.id)"
                      class="flex flex-col h-full justify-between hover:bg-slate-50">
                      <img v-if="activity.imagePath == null" src="../public/image/nophoto.png"
                        alt="Sunset in the mountains" class="w-full h-60 object-cover" />
                      <img v-else :src="activity.imagePath" alt="Sunset in the mountains"
                      ...
                      <div v-if="isRegistered">
                        <div class="px-6 pt-4 pb-2 flex items-end">
                          <span
